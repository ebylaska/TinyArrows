   ...
   for j in range(nion):
      ...
      for i in neighbors1[j]:
         for k in neighbors1[j][ic+1:]:
            ...
            anglebend.append((i,j,k,hybridization,KIJK,ctheta0))
         ic += 1
               ...
               if (i!=j) and (i!=k):
                  for l in neighbors1[k]:
                     if (l!=i) and (l!=j) and (l!=k):
                        ...
                        torsion.append((i,j,k,l,V,n,cnphi))
      ...
      for j in neighbors1[i]:
         ...
         for k in neighbors1[i][ic+1:]:
            for l in neighbors1[i][ic+ic2+2:]:
               ...
               if (abs(KIJKL)>1.0e-6):
                  inversion.append((i,j,k,l,KIJKL,C0,C1,C2))
            ic2 += 1
         ic += 1
      
   return (nion,symbol,rxyz,border_matrix,katom,nkatom_type,katom_type,uffparam_type,preuff,bondstretch,anglebend,torsion,inversion,neighbors1,neighbors12)
...
def uff_anglebend(anglebend,nion,rion):
   ...
   for ab in anglebend:
      ...
      if (denom>1.0e-11):
         ...
         if (ctheta<-1.0): ctheta = -1.0
         stheta = max(math.sqrt(1.0 - ctheta**2),1.0e-8)
...
def uff_torsion(torsion,nion,rion):
   ...
   for tor in torsion:
      ...
      dx_ik = rion[3*i]   - rion[3*k]
      dy_ik = rion[3*i+1] - rion[3*k+1]
      dz_ik = rion[3*i+2] - rion[3*k+2]

      dx_jl = rion[3*j]   - rion[3*l]
      dy_jl = rion[3*j+1] - rion[3*l+1]
      dz_jl = rion[3*j+2] - rion[3*l+2]
      ...
      #b3
      dx_kl = rion[3*k]   - rion[3*l]
      dy_kl = rion[3*k+1] - rion[3*l+1]
      dz_kl = rion[3*k+2] - rion[3*l+2]
      ...
      rr_ijk = 1.0/math.sqrt(dx_ijk*dx_ijk+dy_ijk*dy_ijk+dz_ijk*dz_ijk)
      rr_kjl = 1.0/math.sqrt(dx_kjl*dx_kjl+dy_kjl*dy_kjl+dz_kjl*dz_kjl)

      #n1*n2/(|n1||n2|)
      cphi = rr_ijk*rr_kjl*(dx_ijk*dx_kjl+dy_ijk*dy_kjl+dz_ijk*dz_kjl)
      if (cphi>1.0):  cphi =  1.0
      if (cphi<-1.0): cphi = -1.0
      #sphi = max(math.sqrt(1.0 - cphi**2),1.0e-8)
      sphi = math.sqrt(1.0 - cphi**2)

      phi = math.acos(cphi)
      cnphi = math.cos(n*phi)
      snphi = math.sin(n*phi)
      #print "V,n,PHI=",V,n,phi,cnphi,snphi,0.5*V*(1.0-cnphi0*cnphi)


      ### removed 1/2 because duplicate torsions not included ###
      #E += 0.5*V*(1.0-cnphi0*cnphi)
      E += 1.0*V*(1.0-cnphi0*cnphi)
      if (abs(sphi)>1.0e-8):
         #dEdphi = 0.5*V*cnphi0*n*snphi
         dEdphi = 1.0*V*cnphi0*n*snphi
         aa = -dEdphi/sphi
      else:
         #aa = -0.5*V*cnphi0*n*n
         aa = -1.0*V*cnphi0*n*n
      ...
      dx_a = dx_kjl-cphi*dx_ijk
      dy_a = dy_kjl-cphi*dy_ijk
      dz_a = dz_kjl-cphi*dz_ijk

      dx_b = dx_ijk-cphi*dx_kjl
      dy_b = dy_ijk-cphi*dy_kjl
      dz_b = dz_ijk-cphi*dz_kjl
      ...
      fxj = (rr_ijk*(dy_ik*dz_a-dy_a*dz_ik)-rr_kjl*(dy_kl*dz_b-dy_b*dz_kl))
      fyj = (rr_ijk*(dz_ik*dx_a-dz_a*dx_ik)-rr_kjl*(dz_kl*dx_b-dz_b*dx_kl))
      fzj = (rr_ijk*(dx_ik*dy_a-dx_a*dy_ik)-rr_kjl*(dx_kl*dy_b-dx_b*dy_kl))

      fxk = (rr_kjl*(dy_jl*dz_b-dy_b*dz_jl)-rr_ijk*(dy_ij*dz_a-dy_a*dz_ij))
      fyk = (rr_kjl*(dz_jl*dx_b-dz_b*dx_jl)-rr_ijk*(dz_ij*dx_a-dz_a*dx_ij))
      fzk = (rr_kjl*(dx_jl*dy_b-dx_b*dy_jl)-rr_ijk*(dx_ij*dy_a-dx_a*dy_ij))
...
def uff_LJ(preuff,nkatom_type,katom,neighbors1,neighbors12,nion,rion):
   ...
   for i in range(nion-1):
      for j in range(i+1,nion):
         if (j not in neighbors1[i]) and (j not in neighbors12[i]):
            Dij = preuff[packindx(nkatom_type,katom[i],katom[j])][0]
            xij = preuff[packindx(nkatom_type,katom[i],katom[j])][1]
            dx  = rion[3*i]  -rion[3*j]
            dy  = rion[3*i+1]-rion[3*j+1]
            dz  = rion[3*i+2]-rion[3*j+2]
            r = math.sqrt(dx*dx + dy*dy + dz*dz)
...
bondstretch = uff[9]
print
print "bondstretch=",bondstretch

(E,grad) = uff_bondstretch(bondstretch,nion,rion)
print "E=",E
print "grad=",grad
delta = 0.001
...
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad) = uff_bondstretch(bondstretch,nion,rionp)
   (Em,grad) = uff_bondstretch(bondstretch,nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num

anglebend = uff[10]
print
print "anglebend=",anglebend

(E,grad) = uff_anglebend(anglebend,nion,rion)
print "E=",E
print "grad=",grad
delta = 0.001
grad_num = [0.0]*3*nion
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad) = uff_anglebend(anglebend,nion,rionp)
   (Em,grad) = uff_anglebend(anglebend,nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
...
(E,grad) = uff_torsion(torsion,nion,rion)
print "E=",E
print "grad=",grad
delta = 0.001
grad_num = [0.0]*3*nion
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad) = uff_torsion(torsion,nion,rionp)
   (Em,grad) = uff_torsion(torsion,nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
...
(E,grad) = uff_inversion(inversion,nion,rion)
print "E=",E
print "grad=",grad
delta = 0.001
grad_num = [0.0]*3*nion
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad) = uff_inversion(inversion,nion,rionp)
   (Em,grad) = uff_inversion(inversion,nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
...
print
print "LJ testing"
(E,grad) = uff_LJ(preuff,nkatom_type,katom,neighbors1,neighbors12,nion,rion)
print "E=",E
print "grad=",grad
delta = 0.001
grad_num = [0.0]*3*nion
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad) = uff_LJ(preuff,nkatom_type,katom,neighbors1,neighbors12,nion,rionp)
   (Em,grad) = uff_LJ(preuff,nkatom_type,katom,neighbors1,neighbors12,nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
print
print
print "ALL UFF testing"
(E1,grad1) = uff_bondstretch(bondstretch,nion,rion)
(E2,grad2) = uff_anglebend(anglebend,nion,rion)
(E3,grad3) = uff_torsion(torsion,nion,rion)
(E4,grad4) = uff_inversion(inversion,nion,rion)
(E5,grad5) = uff_LJ(preuff,nkatom_type,katom,neighbors1,neighbors12,nion,rion)
E = E1 + E2 + E3 + E4 + E5
...
while (not done) and (it<55001):
   ...
   print "it,E=",it,E,E1,E2,E3,E4,E5,err,alpha
