...
#                                         #
###########################################
def simple_optimization(uff1,uff2,lmbda,nion,rion0):
   ...
   while (not done) and (it<55001):
      ...
      E = (1.0-lmbda)*E1 + lmbda*E2
      for i in range(3*nion): grad[i] = (1.0 - lmbda)*grad1[i] + lmbda*grad2[i]
      ...
      print "path,it,E=",p,pp,it,E,err,alpha
...
uff-path3
  ...
  Usage: uff-path3 -h xyzsdf_filename bondchange_string npath

     where bondchange_string = "( ([i1,j1,b1],[i2,j2,b2],...), ([k1,l1],[k2,l2],...) )"
     e.g. 
      uff-path3 -h xyzsdf_filename "(([1,4,1], [6,12,1]),())" npath
...
### generate amatrix2 ###
amatrix2 = copy.deepcopy(amatrix)
for ab in bondchanges[0]: ## add bonds ##
   i = ab[0] - 1
   j = ab[1] - 1
   b = ab[2]
   amatrix2[i+j*nion] = b
   amatrix2[j+i*nion] = b
for ab in bondchanges[1]: ## destroy bonds ##
   i = ab[0] - 1
   j = ab[1] - 1
   amatrix2[i+j*nion] = 0
   amatrix2[j+i*nion] = 0
...
uff2 = uff_potential.uff_potential(nion,symbol,amatrix2)
...
for p in range(npath):
   ...
   ### do a crude optimization ###
   pp = p
   (E,rion,grad) = simple_optimization(uff1,uff2,lmbda,nion,rion)

   print "Efinal=",E
   print "rion=",rion
   print "grad  =",grad
   rionp = copy.deepcopy(rion)
   ipathxyz.append(rionp)
   ipathe.append(E)
...
print "writing intial_path.xyz"
with open("intial_path.xyz",'w') as ff: ff.write(ipathstr)

### generate final path ###
pathxyz = copy.deepcopy(ipathxyz)
pathe = copy.deepcopy(ipathe)
for p in range(npath-1,-1,-1):
   lmbda = p*dp
   print "lmbda=",lmbda

   ### do a crude optimization ###
   pp = p
   (E,rion,grad) = simple_optimization(uff1,uff2,lmbda,nion,pathxyz[p])
   if (p<(npath-1)): 
      pp = p + 1
      (E1,rion1,grad1) = simple_optimization(uff1,uff2,lmbda,nion,pathxyz[pp])
      if (E1<E):
         E = E1
         rion = rion1
         grad = grad1
...
print "writing final_path.xyz"
with open("final_path.xyz",'w') as ff: ff.write(pathstr)

print "lmbda  Einitial  Efinal"
for p in range(npath):
   lmbda = p*dp
   print lmbda,ipathe[p],pathe[p]
