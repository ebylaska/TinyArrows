...
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad0) = uff1.bondstretch_egrad(nion,rionp)
   (Em,grad0) = uff1.bondstretch_egrad(nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
for i in range(3*nion): gdiff[i] = grad[i]-grad_num[i]
print
print "gdiff=",gdiff
...
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad0) = uff1.anglebend_egrad(nion,rionp)
   (Em,grad0) = uff1.anglebend_egrad(nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
for i in range(3*nion): gdiff[i] = grad[i]-grad_num[i]
print
...
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad0a) = uff1.torsion_egrad(nion,rionp)
   (Em,grad0b) = uff1.torsion_egrad(nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
for i in range(3*nion): gdiff[i] = grad[i]-grad_num[i]
print
...
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad0) = uff1.inversion_egrad(nion,rionp)
   (Em,grad0) = uff1.inversion_egrad(nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
for i in range(3*nion): gdiff[i] = grad[i]-grad_num[i]
print
...
for i in range(3*nion):
   ...
   rionp[i] += delta
   rionm[i] -= delta
   (Ep,grad0) = uff1.egrad(nion,rionp)
   (Em,grad0) = uff1.egrad(nion,rionm)
   grad_num[i] = (Ep-Em)/(2.0*delta)
print "grad_num=",grad_num
for i in range(3*nion): gdiff[i] = grad[i]-grad_num[i]
print
...
(E,grad) = uff1.bondstretch_egrad(nion,rion)
print "Ebondstretch=",E,uff1.bondstretch_e(nion,rion)
(E,grad) = uff1.anglebend_egrad(nion,rion)
print "Eanglebend  =",E,uff1.anglebend_e(nion,rion)
(E,grad) = uff1.torsion_egrad(nion,rion)
print "Etorsion    =",E,uff1.torsion_e(nion,rion)
(E,grad) = uff1.inversion_egrad(nion,rion)
print "Einversion  =",E,uff1.inversion_e(nion,rion)
(E,grad) = uff1.LJ_egrad(nion,rion)
print "ELJ         =",E,uff1.LJ_e(nion,rion)
