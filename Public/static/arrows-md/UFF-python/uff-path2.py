...
uff-path2
  ...
  Usage: uff-path2 -h sdf_filename bondchange_string npath

     where bondchange_string = "( ([i1,j1,b1],[i2,j2,b2],...), ([k1,l1],[k2,l2],...) )"
     e.g. 
      uff-path2 -h sdf_filename "(([1,4,1], [6,12,1]),())" npath
...
for p in range(npath):
   ...
   ### randomize rion ###
   #for i in range(3*nion): rion[i] += random.uniform(-0.15,0.15)
   ...
   while (not done) and (it<55001):
      ...
      print "path,it,E=",p,it,E,err,alpha
...
print "writing path.xyz"
with open("path.xyz",'w') as ff: ff.write(pathstr)
